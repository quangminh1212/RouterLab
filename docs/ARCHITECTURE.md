# xlabrouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

xlabrouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[xlabrouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.xlabrouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.xlabrouter/usage.json`, `~/.xlabrouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.xlabrouter/db.json`)
- usage stats: `~/.xlabrouter/usage.json`
- request log lines: `~/.xlabrouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[xlabrouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.xlabrouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/xlabrouter && npm run build`
- Build Docker image: `cd /root/dev/xlabrouter && docker build -t xlabrouter .`

...
export default function ProfilePage() {
  ...
  const [gistConfig, setGistConfig] = useState({ enabled: false, hasToken: false, gistId: "", htmlUrl: "", updatedAt: "", tokenSource: "", githubLogin: "", tokenCommand: "" });
  ...
  const [gistTokenForm, setGistTokenForm] = useState({ token: "" });
  ...
  const saveGistToken = async (event) => {
    ...
    try {
      ...
      setGistTokenForm({ token: "" });

      setShowGistTokenForm(false);

      setDbStatus({

        type: data.warning ? "success" : "success",

        message: data.warning

          ? `Access token saved to db.json (warning: ${data.warning})`

          : "Access token saved to db.json",

      });

    } catch (err) {

      setDbStatus({ type: "error", message: err.message || "Failed to save access token" });
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border">
            ...
            <div className="p-3 rounded-lg bg-bg border border-border space-y-3">
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={cn("text-xs px-2 py-1 rounded-full border whitespace-nowrap", gistConfig.hasToken ? "text-green-600 border-green-500/30 bg-green-500/10" : "text-text-muted border-border") }>
                    ...
                    {gistConfig.hasToken ? (gistConfig.tokenSource === "gh-cli" ? "CLI" : "Token") : "Off"}
                ...
                <div className="relative" ref={gistMenuRef}>
                  ...
                  {showGistMenu ? (
                    ...
                    <div className="absolute left-0 top-[calc(100%+8px)] z-20 min-w-[220px] rounded-lg border border-border bg-sidebar p-1 shadow-lg">
                      ...
                      <div role="menuitem" tabIndex={0} className="w-full cursor-pointer text-left px-3 py-2 text-sm rounded-md hover:bg-white/10 transition-colors" onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); }} onClick={() => { setShowGistMenu(false); setShowGistTokenForm((value) => !value); }}>Nhập accesstoken</div>
              ...
              {showGistTokenForm ? (
                ...
                <form onSubmit={saveGistToken} className="grid gap-3 pt-3 border-t border-border/50">

  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:xlabrouter"))
      ...
      if (existingModels.length > 0) {
        ...
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:xlabrouter-0");
        if (legacy?.model) {
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasxlabrouter} loading={restoring}>

...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("xlabrouter/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("xlabrouter/", ""));
    }
  }, [status]);
  ...
  const getConfigStatus = () => {
    ...
    const url = status.config?.provider?.["xlabrouter"]?.options?.baseURL || "";
    const isLocal = url.includes("localhost") || url.includes("127.0.0.1");
    return status.hasxlabrouter && (isLocal || url.includes(baseUrl)) ? "configured" : status.hasxlabrouter ? "other" : "not_configured";
  };
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `xlabrouter/${activeModelToShow}`,
        agent: {
          explorer: {
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if RouterLab is deployed on a remote server.</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 pl-9">
                  <Button variant="secondary" size="sm" onClick={checkStatus} disabled={checking} className="!bg-black/10 !border-border !text-text-main hover:!bg-black/20 dark:hover:!bg-white/10">
                    <span className={`material-symbols-outlined text-[18px] mr-1 ${checking ? "animate-spin" : ""}`}>refresh</span>
                    Refresh
                  </Button>

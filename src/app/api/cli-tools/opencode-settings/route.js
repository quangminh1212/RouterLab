...
const checkOpenCodeInstalled = async () => isCliInstalled("opencode", getConfigPath());
...
const hasxlabrouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["xlabrouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["xlabrouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasxlabrouter: hasxlabrouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("xlabrouter/") ? config.model.replace(/^xlabrouter\//, "") : null,
...
// POST - Apply xlabrouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_xlabrouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing xlabrouter provider entry and its models
    const existingProvider = config.provider["xlabrouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["xlabrouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `xlabrouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `xlabrouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("xlabrouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove xlabrouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["xlabrouter"]?.models) {
      delete config.provider["xlabrouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["xlabrouter"].models).length === 0) {
        delete config.provider["xlabrouter"];
        if (config.model?.startsWith("xlabrouter/")) delete config.model;
      } else if (config.model === `xlabrouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["xlabrouter"].models);
        config.model = `xlabrouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire xlabrouter provider
      if (config.provider) delete config.provider["xlabrouter"];
      if (config.model?.startsWith("xlabrouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("xlabrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "xlabrouter settings removed from OpenCode",
    });
  } catch (error) {

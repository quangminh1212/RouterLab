...
const hasxlabrouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "xlabrouter");
};

const getxlabrouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "xlabrouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getxlabrouterEntry(config);
...
export async function POST(request) {
  try {
    ...
    // Replace existing xlabrouter entry or append
    const idx = config.findIndex((e) => e.name === "xlabrouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove xlabrouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "xlabrouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "xlabrouter removed from Copilot config",
    });
  } catch (error) {

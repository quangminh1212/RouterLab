...
// Check if settings has xlabrouter customModels
const hasxlabrouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:xlabrouter"));
};
...
// POST - Update xlabrouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing xlabrouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:xlabrouter"));
...
// DELETE - Remove xlabrouter customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove xlabrouter customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:xlabrouter"));

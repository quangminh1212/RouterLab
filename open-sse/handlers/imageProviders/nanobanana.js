...
export default {
  ...
  async parseResponse(response, { headers }) {
    ...
    if (submitData?.image || submitData?.images || submitData?.data?.[0]?.b64_json) {
      return submitData;
    }
  ...
  normalize: (responseBody, prompt) => {
    ...
    if (Array.isArray(responseBody?.images) && responseBody.images.length > 0) {
      return {
        created: nowSec(),
        data: responseBody.images.filter(Boolean).map((image) => ({ b64_json: image, revised_prompt: prompt })),
      };
    }

...
<div align="center">
  <img src="../images/xlabrouter.png?1" alt="Bảng điều khiển xlabrouter" width="800"/>
  ...
  **Nhà cung cấp AI Miễn cho OpenClaw.**
  ...
  [🚀 Bắt đầu nhanh](#-quick-start) • [💡 Tính năng](#-key-features) • [📖 Cài đặt](#-setup-guide) • [🌐 Website](https://xlabrouter.com)
</div>

---

## 🤔 Tại sao chọn xlabrouter?
...
**xlabrouter giải quyết vấn đề này:**
...
Gói kho lưu trữ này là riêng tư (`xlabrouter-app`), vì vậy việc thực thi nguồn/Docker là đường dẫn phát triển cục bộ dự kiến.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - xlabrouter + Claude Code MIỄN PHÍ
  
[![Thiết lập xlabrouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập xlabrouter
...
xlabrouter hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → xlabrouter dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân xlabrouter **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của xlabrouter

**Thực tế Thanh toán xlabrouter:**

✅ **Phần mềm xlabrouter = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **xlabrouter không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, xlabrouter chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị xlabrouter tính phí không?</b></summary>

**Không.** xlabrouter là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, xlabrouter chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân xlabrouter** → **Không bao giờ tính phí bất cứ thứ gì, ever**

xlabrouter là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
xlabrouter chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và xlabrouter giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải xlabrouter.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - xlabrouter giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của xlabrouter ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và xlabrouter sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. xlabrouter theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển xlabrouter]
...
> **Lưu ý:** OpenClaw chỉ hoạt động với xlabrouter cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
# Hoặc sử dụng PM2
npm install -g pm2
pm2 start --name xlabrouter -- start
pm2 save
pm2 startup
```
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | `xlabrouter-default-secret-change-me` | Bí mật ký JWT cho cookie xác thực bảng điều khiển (**thay đổi trong production**) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.xlabrouter` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
...
| `CLOUD_URL` | `https://xlabrouter.com` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://xlabrouter.com` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
- Lịch sử sử dụng và log: `~/.xlabrouter/usage.json` và `~/.xlabrouter/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.xlabrouter` và độc lập với `DATA_DIR`.
...
**Token OAuth hết hạn**
- Tự động làm mới bởi xlabrouter
...
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `https://xlabrouter.com`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`https://xlabrouter.com/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime xlabrouter cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Gửi một yêu cầu tương thích OpenAI trực tiếp đến endpoint đám mây (`https://xlabrouter.com/v1/chat/completions`) với mô hình/key được cung cấp.
...
- Endpoint đám mây công khai (`https://xlabrouter.com/v1/chat/completions`):
...
- **GitHub**: [github.com/decolua/9](https://github.com/decolua/xlabrouter)
...
Cảm ơn tất cả những người đã đóng góp giúp xlabrouter tốt hơn!

[![Contributors](https://contrib.rocks/image?repo=decolua/xlabrouter&max=100&columns=20&anon=1)](https://github.com/decolua/xlabrouter/graphs/contributors)

---

## 📊 Star Chart

[![ Chart](https://starchart.cc/decolua/xlabrouter.svg?variant=adaptive)](https://starchart.cc/decolua/xlabrouter)

<div align="center">
  ...
  **OpenClaw 的免费 AI 提供商。**
  ...
  [🚀 快速开始](#-quick-start) • [💡 特性](#-key-features) • [📖 设置](#-setup) • [🌐 网站](https://xlabrouter.com)
</div>
...
此仓库包是私有的（`xlabrouter-app`），因此源码/Docker 执行是预期的本地开发路径。
...
### 📺完整设置指南 - xlabrouter + Claude Code 免费
...
- 您的 CLI 工具发送 OpenAI 格式 → xlabrouter 转换 → 提供商接收原生格式
...
✅ **xlabrouter 软件 = 永远免费**开源，从不收费）  
✅ **仪表板“成本” = 仅显示/追踪**（非实际账单）  
✅ **您直接向提供商付款**（订阅或 API 费用）  
✅ **免费提供商保持免费**（iFlow, Kiro, Qwen = $0 无限制）  
❌ **xlabrouter 从不发送发票**或向您的卡收费
...
- **廉价提供商**（GLM, MiniMax）：直接向他们付款，xlabrouter 只是路由
- **免费**（iFlow, Kiro, Qwen）：真正永远免费，没有隐藏费用
- **xlabrouter**：从不收取任何费用，永远
...
仪表板追踪您的 Token 使用情况，并显示**估算成本**，就像您直接使用付费 API 一样。这**不是实际计费** - 它是一个参考，显示您通过 xlabrouter 使用免费模型或现有订阅节省了多少。
...
<details>
<summary><b>💳 xlabrouter 会向我收费吗？</b></summary>

**不会。** 9 是免费的开源软件，在您自己的计算机上运行。它从不向您收费。

**您只需支付：**
- ✅ **订阅提供商**（Claude Code $20/月, Codex $20-200/月）→ 在他们的网站上直接向他们付款
- ✅ **廉价提供商**（GLM, MiniMax）→ 直接向他们付款，xlabrouter 只是路由您的请求
- ❌ **xlabrouter 本身** → **从不收取任何费用，永远**

xlabrouter 是本地代理/路由器。它没有您的信用卡，不能发送发票，也没有计费系统。完全免费的软件。
...
Router 只是将您的请求路由到它们 - 没有“陷阱”或未来计费。它们是真正的免费服务，xlabrouter 使它们易于使用并支持回退。

**注意：** 一些订阅提供商（Antigravity, GitHub Copilot）可能有免费预览期，后来可能变成付费，但这会由这些提供商明确宣布，而不是 xlabrouter。
...
**您在控制中：** 在仪表板中设置每个提供商的支出限制，xlabrouter 会遵守它们。
...
**专业提示：** 使用 Opus 处理复杂任务，Sonnet 追求速度。xlabrouter 追踪每个模型的配额！
...
> **注意：** OpenClaw 仅适用于本地 xlabrouter。使用 `127.0.0.1` 而不是 `localhost` 以避免 IPv6 解析问题。
...
| 变量 | 默认值 | 描述 |
|----------|---------|-------------|
| `JWT_SECRET` | `xlabrouter-default-secret-change-me` | 仪表板认证 cookie 的 JWT 签名密钥（**生产环境中请更改**） |
| `INITIAL_PASSWORD | `123456` | 当没有保存的哈希时的首次登录密码 |
| `DATA_DIR` | `~/.xlabrouter` | 主应用数据库位置（`db.json`） |
...
| `CLOUD_URL` | `https://xlabrouter.com` | 服务器端云同步端点基础 URL |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | 向后兼容/公共基础 URL（服务器运行时优先使用 `BASE_URL`） |
| `NEXT_PUBLIC_CLOUD_URL` | `https://xlabrouter.com` | 向后兼容/公共云 URL（服务器运行时优先使用 `CLOUD_URL`） |
...
- 使用历史和日志：`~/.xlabrouter/usage.json` 和 `~/.xlabrouter/log.txt`，由 `src/lib/usageDb.js` 管理。
...
- 验证 `CLOUD_URL` 指向您预期的云端端点（例如：`https://xlabrouter.com`）
- 尽可能保持 `NEXT_PUBLIC_*` 值与服务器端值一致。

**云端端点 `stream=false` 返回 500（`Unexpected token 'd'...`）**
- 症状通常出现在公共云端端点（`https://xlabrouter.com/v1`）的非流式调用上。
- 根本原因：上游返回 SSE 负载（`data: ...`）而客户端期望 JSON。
- 变通方法：对云端直接调用使用 `stream=true`。
- 当上游返回 `text/event-stream` 时，本地 xlabrouter 运行时包含 SSE→JSON 回退用于非流式调用。
...
- `tester/security/test-cloud-openai-compatible.sh`
  - 使用提供的模型/密钥向云端端点（`https://xlabrouter.com/v1/chat/completions`）发送直接的 OpenAI 兼容请求。
...
- 公共云端端点（`https://xlabrouter.com/v1/chat/completions`）：
...
- **网站**：[xlabrouter.com](https://xlabrouter.com)
- **GitHub**：[github.com/decolua/xlabrouter](https://github.com/decolua/xlabrouter)
- **问题**：[github.com/decolua/xlabrouter/issues](https://github.com/decolua/xlabrouter/issues)

---

## 👥 贡献者

感谢所有帮助让 xlabrouter 变得更好的贡献者！

[![Contributors](https://contrib.rocks/image?repo=decolua/router&max=100&columns=20&anon=1)](https://github.com/decolua/xlabrouter/graphs/contributors)
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — xlabrouter 的全功能 TypeScript 分支。添加了 36+ 提供商、4 层自动回退、多模态 API（图像、嵌入、音频、TTS）、熔断器、语义缓存、LLM 评估和精美的仪表板。8+ 单元测试。通过 npm 和 Docker 可用。
